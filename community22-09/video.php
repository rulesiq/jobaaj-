  ...
  <?php 
    ...
    // if($user['com_category']!='0' && $user['id']!='')
    //     $skill = $user['com_category']; 
...
var offset = 0;
  ...
  function addPost(){
       ...
       cat = $(".post-cat").val();
        ...
        let postData = new FormData($('#postForm')[0]);

// Create a Blob object from the data URL
        fetch(global_thumb_url).then(response => response.blob()).then(blob => {
    // Append the Blob as a file to the FormData object
    postData.append('posted_thumb_url', blob, 'image.png');
    ...
    <form enctype="multipart/form-data" id="postForm" accept-charset="UTF-8" >
       ...
       <div class="img-choosen" style="">
        ...
        <div class="img-selected">
         ...
         <input type="hidden"  name="videoPost" value="true"/>
         ...
         <video style="display:none;" id="my_video" controls crossOrigin="Anonymous">
          </video>
          <canvas style="display:none" style="height:200px;width:100px" id="thecanvas"></canvas>
         <img style="height:200px;width:100px;display:none;" id="thumbnail_img" class="" alt="Right click to save" />
    ...
    <div class="modal-content">
      ...
      <div class="modal-header" style="display:flex;">
        <h4 class="modal-title" id="myModalLabel">Update Category<br>
                <span style="font-size:.7rem;">Choose from the below categories to know what is happening <br/>in the respective fields.</span></h4>
...
.onchange = function(event) {
  ...
  document.getElementById("my_video").src = blobURL;
  
  getThumb();
}

let global_thumb_url;
...
function getThumb(){
    ...
    // Reduce the dimensions of the compressed canvas
    const maxWidth = 400;
    const maxHeight = 700;
    const scaleFactor = Math.min(maxWidth / thecanvas.width, maxHeight / thecanvas.height);
    const newWidth = thecanvas.width * scaleFactor;
    const newHeight = thecanvas.height * scaleFactor;

    compressedCanvas.width = newWidth;
    compressedCanvas.height = newHeight;

    // Draw the original image onto the compressed canvas
    compressedContext.drawImage(thecanvas, 0, 0, newWidth, newHeight);
    // Set the image source to the compressed canvas's data URL
    img.setAttribute('src', compressedCanvas.toDataURL('image/jpeg', 0.8));
    
    global_thumb_url = compressedCanvas.toDataURL();
  }
