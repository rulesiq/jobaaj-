...
@media only screen and (max-width: 767px) {
  ...
  <script>
      ...
      function getPosts(cat) {
          ...
          $(".nav-link").parent().removeClass('nav-item-active');
          $(`.cat${cat}`).parent().addClass('nav-item-active');
          ...
          loadData(cat,search_key,offset);
    ...
    //             console.log(e.closest('span').textContent);
    //         });
    ...
    //   });
      ...
      function addPost() {
          ...
          if(cat==null) {
          showToast('Select category!', 2000);
          $(".post-cat").focus();
          return;
          }
          ...
          $.ajax({
              ...
              success: function(result) {
                  ...
                  if (result == 1) {
                      ...
                      if ($("#postInput").val() == '0') {
                          showToast('Post added for review!', 3000);
                          myPosts();
                      } else {
                          showToast('Post edited & send for review!', 3000);
                          location.reload();
                      }
                      showToast('Post added for review!', 3000);
                  }
              ...
              <div class="modal-body">
                  <form id="showCat" style="padding:0 1rem">
                      ...
                      <ul style="padding-left:0px !important;" class="menu_sidebar metisMenu">
                          <?php
                            //
                            $q = mysqli_query($db, "select * from com_category where id not in (-1,-2,10) order by position");
                            $i = 0;
                            while ($c = mysqli_fetch_array($q)) {
                                ++$i; ?>
                              <li class="nav-item">
  ...
  <script>
      ...
      $("#thumb_file").change(function() {
          ...
          for (var i = 0; i < total_file; i++) {
              $('#img-preview').append("<div style='width:100%'><img class='pimg' src='" + URL.createObjectURL(event.target.files[i]) + "'></div>");
          }
  ...
  if(isset($_GET['myPost'])){
      ...
      echo "<script>
      setTimeout(function() { myPosts() } , 250);
      
      setTimeout(function() { shareCertificate('$id')  }, 500);</script>";
  }
  ?>
